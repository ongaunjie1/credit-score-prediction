...
income = st.sidebar.number_input('Annual Income', min_value=10000.0, max_value=1000000000.0, value=10000.0)
num_of_bank_acc = st.sidebar.number_input('Number of Bank Accounts Owned', min_value=1 ,max_value=12, value=1)
num_credit_card = st.sidebar.number_input('Number of Credit Cards Owned', min_value=1 ,max_value=12, value=1)
interest = st.sidebar.number_input('Credit Cards Interest rate (%)',min_value=1.0, max_value=33.0, value=1.0, step=0.1)
delay_from_due_date = st.sidebar.number_input('Days Delayed Since Due Date for Payment', min_value=0, max_value=60, value=1)
outstanding_debt = st.sidebar.number_input('Outstanding Debt ($)', min_value=0.0, max_value=100000.0, value=0.0)
credit_util = st.sidebar.number_input('Credit Card Utilization)', min_value=0.0, max_value=100.0, value=0.0)
credit_history = st.sidebar.number_input('Credit History Age', min_value=0.1, max_value=100.12, value=0.1)
installment_per_month = st.sidebar.number_input('Credit Card Installment per Month ($)', min_value=0.0, max_value=100000.0, value=0.0)
amount_invested_month = st.sidebar.number_input('Amount Invested Per Month ($)', min_value=0.0, max_value=100000.0, value=0.0)
